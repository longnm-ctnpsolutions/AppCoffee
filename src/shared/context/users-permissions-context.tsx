"use client";

import * as React from 'react';
import type { Permission } from '@/types/permissions.types';
import type { TableState } from '@/types/odata.types';
import { getUserPermissionsWithOData, UserPermissionsQueryResult } from '../api/services/users/users-odata.service';
import { addUserPermission, deleteUserPermissionId, getUserPermission } from '../api/services/permissions/permissions.service';


interface UserPermissionsState {
    userPermissions: Permission[];
    isLoading: boolean;
    allPermissions: Permission[];
    isActionLoading: boolean;
    isAllLoading: boolean;
    error: string | null;
    totalCount: number;
    hasMore: boolean;
    searchTerm: string;
    selectedUserPermission: Permission | null;
}

type UserPermissionsAction =
    | { type: 'FETCH_INIT'; }
    | { type: 'FETCH_SUCCESS'; payload: UserPermissionsQueryResult; }
    | { type: 'FETCH_FAILURE'; payload: string; }
    | { type: 'FETCH_ALL_INIT' }
    | { type: 'FETCH_ALL_SUCCESS'; payload: Permission[] }        // thêm
    | { type: 'FETCH_ALL_FAILURE'; payload: string }
    | { type: 'ADD_SUCCESS'; payload: Permission; }
    | { type: 'ADD_MANY_SUCCESS'; payload: { ids: string[]; }; }
    | { type: 'REMOVE_SUCCESS'; payload: { id: string; }; }
    | { type: 'SET_ACTION_LOADING'; payload: boolean; }
    | { type: 'SET_ERROR'; payload: string | null; }
    | { type: 'SET_SEARCH_TERM'; payload: string; }
    | { type: 'CLEAR_SEARCH'; }
    | { type: 'CLEAR_SELECTED_USER'; };


const userPermissionsReducer = (state: UserPermissionsState, action: UserPermissionsAction): UserPermissionsState => {
    switch (action.type) {
        case 'FETCH_INIT':
            return { ...state, isLoading: true, error: null };

        case 'FETCH_SUCCESS':
            return {
                ...state,
                isLoading: false,
                userPermissions: action.payload.userPermissions,
                totalCount: action.payload.totalCount,
                hasMore: action.payload.hasMore
            };

        case 'FETCH_FAILURE':
            return { ...state, isLoading: false, error: action.payload };

        case 'ADD_SUCCESS':
            return {
                ...state,
                userPermissions: [action.payload, ...state.userPermissions],
                totalCount: state.totalCount + 1
            };

        case "ADD_MANY_SUCCESS":
            return {
                ...state,
                userPermissions: state.userPermissions.filter(
                    p => !action.payload.ids.includes(p.id)
                ),
                totalCount: state.totalCount + action.payload.ids.length,
                isActionLoading: false,
                selectedUserPermission: state.selectedUserPermission && action.payload.ids.includes(state.selectedUserPermission.id)
                    ? null
                    : state.selectedUserPermission
            };

        case 'REMOVE_SUCCESS':
            return {
                ...state,
                userPermissions: state.userPermissions.filter(userPermissions => userPermissions.id !== action.payload.id),
                totalCount: state.totalCount - 1,
                isActionLoading: false,
                selectedUserPermission: state.selectedUserPermission?.id === action.payload.id ? null : state.selectedUserPermission
            };

        case 'SET_ACTION_LOADING':
            return { ...state, isActionLoading: action.payload };

        case 'SET_ERROR':
            return { ...state, error: action.payload, isLoading: false, isActionLoading: false };

        case 'SET_SEARCH_TERM':
            return { ...state, searchTerm: action.payload };

        case 'CLEAR_SEARCH':
            return { ...state, searchTerm: '' };

        case 'FETCH_ALL_INIT':
            return { ...state, isAllLoading: true, error: null };

        case 'FETCH_ALL_SUCCESS':
            return { ...state, isAllLoading: false, allPermissions: action.payload };

        case 'FETCH_ALL_FAILURE':
            return { ...state, isAllLoading: false, error: action.payload };

        default:
            return state;
    }
};

// Initial state
const initialState: UserPermissionsState = {
    userPermissions: [],
    selectedUserPermission: null,
    allPermissions: [],
    isAllLoading: false,
    isLoading: false,
    isActionLoading: false,
    error: null,
    totalCount: 0,
    hasMore: false,
    searchTerm: '',
};

const UserPermissionsStateContext = React.createContext<UserPermissionsState | undefined>(undefined);
const UserPermissionsDispatchContext = React.createContext<React.Dispatch<UserPermissionsAction> | undefined>(undefined);


interface UserPermissionsProviderProps {
    children: React.ReactNode;
    debounceDelay?: number;
}

export const UserPermissionsProvider: React.FC<UserPermissionsProviderProps> = ({
    children
}) => {
    const [state, dispatch] = React.useReducer(userPermissionsReducer, initialState);

    return (
        <UserPermissionsStateContext.Provider value={state}>
            <UserPermissionsDispatchContext.Provider value={dispatch}>
                {children}
            </UserPermissionsDispatchContext.Provider>
        </UserPermissionsStateContext.Provider>
    );
};

// HOOKS ĐỂ ACCESS RIÊNG BIỆT STATE VÀ DISPATCH
export const useUserPermissionsState = (): UserPermissionsState => {
    const context = React.useContext(UserPermissionsStateContext);
    if (context === undefined) {
        throw new Error('useUserPermissionsState must be used within a UserPermissionsProvider');
    }
    return context;
};

export const useUserPermissionsDispatch = (): React.Dispatch<UserPermissionsAction> => {
    const context = React.useContext(UserPermissionsDispatchContext);
    if (context === undefined) {
        throw new Error('useUserPermissionsDispatch must be used within a UserPermissionsProvider');
    }
    return context;
};

export const useUserPermissionsStateActions = (userId: string, debounceDelay: number = 300) => {
    const state = useUserPermissionsState();
    const dispatch = useUserPermissionsDispatch();

    // Refs để track state và prevent unnecessary calls
    const currentTableStateRef = React.useRef<TableState | null>(null);
    const fetchTimeoutRef = React.useRef<NodeJS.Timeout | null>(null);
    const isInitializedRef = React.useRef(false);
    const lastFetchParamsRef = React.useRef<string>('');

    // Stable fetch function với ref để prevent recreation
    const fetchUserPermissionsInternal = React.useCallback(async (userId: string, tableState: TableState, searchQuery: string) => {
        // Prevent duplicate calls bằng cách compare parameters
        const currentParams = JSON.stringify({ tableState, searchQuery });
        if (lastFetchParamsRef.current === currentParams) {
            console.log('🚫 Duplicate API call prevented');
            return;
        }

        console.log('🔥 fetchUserPermissionsInternal called with:', { userId, tableState, searchQuery });
        lastFetchParamsRef.current = currentParams;

        dispatch({ type: 'FETCH_INIT' });
        try {
            const result = await getUserPermissionsWithOData(userId, tableState, searchQuery);
            dispatch({ type: 'FETCH_SUCCESS', payload: result });
        } catch (error) {
            const message = error instanceof Error ? error.message : 'An unknown error occurred';
            dispatch({ type: 'FETCH_FAILURE', payload: message });
        } finally {
            // Reset after a short delay để allow cho next legitimate call
            setTimeout(() => {
                lastFetchParamsRef.current = '';
            }, 100);
        }
    }, [dispatch]);

    // Main fetch function - CHỈ update ref, KHÔNG trigger search effect
    const fetchUserPermissions = React.useCallback(async (userId: string, tableState: TableState) => {
        console.log('📋 fetchPermissions user called');

        // Clear existing timeout
        if (fetchTimeoutRef.current) {
            clearTimeout(fetchTimeoutRef.current);
        }

        // Update ref KHÔNG trigger re-render
        currentTableStateRef.current = tableState;

        // Call immediately cho non-search requests
        await fetchUserPermissionsInternal(userId, tableState, state.searchTerm);
    }, [fetchUserPermissionsInternal, state.searchTerm]);

    // Main fetch function - CHỈ update ref, KHÔNG trigger search effect

    const fetchAllUserPermissions = React.useCallback(async () => {
        dispatch({ type: 'FETCH_ALL_INIT' });
        try {
          const data = await getUserPermission(userId);
          dispatch({ type: 'FETCH_ALL_SUCCESS', payload: data });
        } catch (err) {
          dispatch({ type: 'FETCH_ALL_FAILURE', payload: (err as Error).message });
        }
    }, [fetchUserPermissionsInternal, dispatch]);

    const addUserPermissionAction = React.useCallback(
        async (permissionIds: string[]) => {
            console.log("➕ Adding permissions:", permissionIds);
            dispatch({ type: "SET_ACTION_LOADING", payload: true });

            try {
                await addUserPermission(userId, permissionIds);
                await fetchAllUserPermissions()
                dispatch({
                    type: "ADD_MANY_SUCCESS",
                    payload: { ids: permissionIds }
                });
                dispatch({ type: "SET_ACTION_LOADING", payload: false });

                if (currentTableStateRef.current) {
                    await fetchUserPermissionsInternal(userId, currentTableStateRef.current, state.searchTerm);
                }

                console.log("✅ Permissions added successfully");

                return true;
            } catch (error) {
                const message =
                    error instanceof Error ? error.message : "Failed to add permissions";
                dispatch({ type: "SET_ERROR", payload: message });
                console.error("❌ Add permissions failed:", error);
                throw error;
            } finally {
                dispatch({ type: "SET_ACTION_LOADING", payload: false });
            }
        },
        [userId, dispatch, fetchUserPermissionsInternal, state.searchTerm]
    );


    const deleteUserPermissionAction = React.useCallback(
        async (userId: string, permissionId: string) => {
            console.log("🗑️ Deleting permission:", permissionId);
            dispatch({ type: "SET_ACTION_LOADING", payload: true });

            try {
                await deleteUserPermissionId(userId, permissionId);
                await fetchAllUserPermissions()
                dispatch({ type: "REMOVE_SUCCESS", payload: { id: permissionId } });
                console.log("✅ Permission user deleted successfully:", permissionId);
            } catch (error) {
                const message =
                    error instanceof Error ? error.message : "Failed to delete permission";
                dispatch({ type: "SET_ERROR", payload: message });
                console.error("❌ Delete permission failed:", error);
                throw error;
            } finally {
                dispatch({ type: "SET_ACTION_LOADING", payload: false });
            }
        },
        [dispatch]
    );


    // SINGLE useEffect cho debounced search - CHỈ handle search term changes
    React.useEffect(() => {
        // Skip nếu chưa có table state hoặc chưa initialized
        if (!currentTableStateRef.current || !isInitializedRef.current) return;

        console.log('🔍 Search term changed, setting up debounce:', state.searchTerm);

        // Clear existing timeout
        if (fetchTimeoutRef.current) {
            clearTimeout(fetchTimeoutRef.current);
        }

        // Set up debounced search
        fetchTimeoutRef.current = setTimeout(() => {
            console.log('🔍 Debounced search triggered:', state.searchTerm);
            fetchUserPermissionsInternal(userId, currentTableStateRef.current!, state.searchTerm);
        }, debounceDelay);

        return () => {
            if (fetchTimeoutRef.current) {
                clearTimeout(fetchTimeoutRef.current);
            }
        };
    }, [state.searchTerm, fetchUserPermissionsInternal, debounceDelay, userId]);

    // Initialization effect - CHỈ chạy 1 lần
    React.useEffect(() => {
        if (!isInitializedRef.current) {
            isInitializedRef.current = true;
            console.log('✅ PermissionsActions initialized');
        }
    }, []);

    // Cleanup on unmount
    React.useEffect(() => {
        return () => {
            if (fetchTimeoutRef.current) {
                clearTimeout(fetchTimeoutRef.current);
            }
        };
    }, []);

    // Search actions
    const setSearchTerm = React.useCallback((term: string) => {
        console.log('🔍 Setting search term:', term);
        dispatch({ type: 'SET_SEARCH_TERM', payload: term });
    }, [dispatch]);

    const clearSearch = React.useCallback(() => {
        dispatch({ type: 'CLEAR_SEARCH' });
    }, [dispatch]);

    const isSearching = React.useMemo(() => {
        return state.searchTerm.trim().length > 0;
    }, [state.searchTerm]);

    return {
        // State (for easy access)
        ...state,

        // Search
        setSearchTerm,
        clearSearch,
        isSearching,

        // Actions
        fetchUserPermissions,
        addUserPermissionAction,
        fetchAllUserPermissions,

        deleteUserPermissionAction
    };
};

// CONVENIENCE HOOK CHO NHỮNG COMPONENT CHỈ CẦN READ STATE
export const usePermissionsData = () => {
    const state = useUserPermissionsState();

    return {
        userPermissions: state.userPermissions,
        allPermissions: state.allPermissions,
        isAllLoading: state.isAllLoading,
        isLoading: state.isLoading,
        isActionLoading: state.isActionLoading,
        error: state.error,
        totalCount: state.totalCount,
        hasMore: state.hasMore,
        searchTerm: state.searchTerm,
        isSearching: state.searchTerm.trim().length > 0,
    };
};
