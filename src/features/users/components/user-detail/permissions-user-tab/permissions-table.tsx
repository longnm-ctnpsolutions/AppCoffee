...
const PermissionsTable: React.FC<PermissionsTableProps> = ({
    ...
    userId,
    initialPageSize = 20,
    canUserPermissionsDelete = false,
    searchTerm,
    setSearchTerm,
    originalData
}) => {
    ...
    const tableConfig: ColumnConfig[] = React.useMemo(() => {
        const baseConfig: ColumnConfig[] = [
            ...
            createEnhancedColumnConfig('clientName', 3, 3, 400, 300, {
                contentBased: false,
                hideAt: 'md'
            })
        ];

        if (canUserPermissionsDelete) {
            baseConfig.push(
    ...
    const columns: ColumnDef<Permission>[] = React.useMemo(() => {
        ...
        return baseColumns;
    }, [canUserPermissionsDelete, onDeletePermission, isLoading]);
